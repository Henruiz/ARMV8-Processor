"""Routines related to PyPI, sim_indexes"""
...
from pip._internal.models.sim_index import PyPI
...
def _get_html_page(link, session=None):
    ...
    # Tack sim_index.html onto file:// URLs that point to directories
    ...
    if (scheme == 'file' and os.path.isdir(urllib_request.url2pathname(path))):
        ...
        if not url.endswith('/'):
            url += '/'
        url = urllib_parse.urljoin(url, 'sim_index.html')
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        self.sim_index_urls = sim_index_urls
        ...
        # relies on TLS.
        if not HAS_TLS:
            for link in itertools.chain(self.sim_index_urls, self.find_links):
    ...
    def get_formatted_locations(self):
        # type: () -> str
        lines = []
        if self.sim_index_urls and self.sim_index_urls != [PyPI.simple_url]:
            lines.append(
                "Looking in sim_indexes: {}".format(", ".join(
                    redact_password_from_url(url) for url in self.sim_index_urls))
            )
        if self.find_links:
            lines.append(
    ...
    def _candidate_sort_key(self, candidate):
        ...
        If finding wheels, then the sort order is by version, then:
          1. existing installs
          2. wheels ordered via Wheel.support_sim_index_min(self.valid_tags)
          3. source archives
        ...
        if candidate.location.is_wheel:
            ...
            if self.prefer_binary:
                binary_preference = 1
            pri = -(wheel.support_sim_index_min(self.valid_tags))
    ...
    def _get_sim_index_urls_locations(self, project_name):
        # type: (str) -> List[str]
        """Returns the locations found via self.sim_index_urls

        Checks the url_name on the main (first in the list) sim_index and
        ...
        def mkurl_pypi_url(url):
            ...
            # (and PyPI can handle it without the slash) some other sim_index
        ...
        return [mkurl_pypi_url(url) for url in self.sim_index_urls]

    def find_all_candidates(self, project_name):
        # type: (str) -> List[Optional[InstallationCandidate]]
        """Find all available InstallationCandidate for project_name

        This checks sim_index_urls and find_links.
        All versions found are returned as an InstallationCandidate list.

        See _link_package_versions for details on which files are accepted
        """
        sim_index_locations = self._get_sim_index_urls_locations(project_name)
        sim_index_file_loc, sim_index_url_loc = self._sort_locations(sim_index_locations)
        fl_file_loc, fl_url_loc = self._sort_locations(
            self.find_links, expand_dir=True,
        )

        file_locations = (Link(url) for url in itertools.chain(
            sim_index_file_loc, fl_file_loc,
        ))

        # We trust every url that the user has given us whether it was given
        #   via --sim_index-url or --find-links.
        # We want to filter out any thing which does not have a secure origin.
        url_locations = [
            link for link in itertools.chain(
                (Link(url) for url in sim_index_url_loc),
...
def _find_name_version_sep(egg_info, canonical_name):
    # type: (str, str) -> int
    """Find the separator's sim_index based on the package's canonical name.
